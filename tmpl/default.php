...
$insert_header         = $params->get("insert_header",1);

// If need limit to a article
if ( $limit_id ) {

	$app            = JFactory::getApplication();
	$menu           = $app->getMenu();
	$component_type = $menu->getActive()->type;
	$component_id   = $menu->getActive()->component_id;

	$db    = JFactory::getDBO();
	$query = 'SELECT element AS context FROM #__extensions WHERE extension_id = ' . $component_id;
	$db->setQuery($query);  
...
// Limit the display for a item article
if ( ($limit_id != "" && $itemid == $limit_id) || $limit_id == "" ):
